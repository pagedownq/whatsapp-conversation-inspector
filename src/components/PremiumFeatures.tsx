...
const PremiumFeatures = () => {
  ...
  return <>
      <div className="flex justify-center mt-4 mb-6 px-4">
        <motion.button onClick={() => setShowDialog(true)} className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 rounded-full bg-gradient-to-r from-amber-200/30 to-amber-500/30 text-purple-800 border border-amber-300/50 hover:bg-amber-200/40 transition-all shadow-soft hover:shadow-lg" whileHover={{
        scale: 1.03
      }} whileTap={{
        scale: 0.98
      }}>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="bg-gradient-to-b from-white to-amber-50 border border-amber-200/50 max-w-[95vw] sm:max-w-2xl w-full">
          ...
          <DialogFooter className="flex flex-col gap-2 sm:flex-row sm:justify-between mt-2">
            <Button variant="outline" onClick={() => setShowDialog(false)} className="border-amber-200/50 text-purple-800 w-full sm:w-auto">
              <X className="h-4 w-4 mr-2" />
              Kapat
            </Button>
            {!user ? <Button onClick={handleLoginClick} className="bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-purple-900 border border-amber-300 w-full sm:w-auto">
                <Crown className="h-4 w-4 mr-2" />
                Giriş Yap ve Premium Al
              </Button> : <Button onClick={() => navigate('/premium')} className="bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-purple-900 border border-amber-300 w-full sm:w-auto">
                <Crown className="h-4 w-4 mr-2" />
                Premium Üyelik Al
              </Button>}
          </DialogFooter>
