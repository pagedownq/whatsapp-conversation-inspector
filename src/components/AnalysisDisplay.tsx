...
const StatsCard: React.FC<StatsCardProps> = ({
  ...
  return <motion.div className="bg-card rounded-2xl p-5 shadow-soft" whileHover={{
    y: -3,
    transition: {
      duration: 0.2
    }
...
const MediaStatsCard: React.FC<{
  ...
  return <div className="bg-secondary/50 rounded-xl p-4 flex flex-col items-center">
      <div className="rounded-full p-2 mb-2" style={{
...
const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({
  ...
  const chartProgress = useProgressAnimation({
    duration: 1000,
    delay: 300
  });
  useEffect(() => {
  ...
  const getParticipantMediaData = (participant: string) => [{
    name: 'Fotoğraflar',
    value: stats.participantStats[participant].mediaStats.images
  }, {
    name: 'Videolar',
    value: stats.participantStats[participant].mediaStats.videos
  }, {
    name: 'Belgeler',
    value: stats.participantStats[participant].mediaStats.documents
  }, {
    name: 'Linkler',
    value: stats.participantStats[participant].mediaStats.links
  }, {
    name: 'Çıkartmalar',
    value: stats.participantStats[participant].mediaStats.stickers
  }, {
    name: 'GIFler',
    value: stats.participantStats[participant].mediaStats.gifs
  }, {
    name: 'Sesler',
    value: stats.participantStats[participant].mediaStats.audio
  }];
  ...
  return <motion.div className="w-full max-w-6xl mx-auto" initial={{
    opacity: 0
  }} animate={{
    opacity: 1
  }} transition={{
    duration: 0.5
      ...
      {isMobile && <div className="flex items-center justify-center gap-2 mb-4 p-2 bg-secondary/50 rounded-lg text-sm">
      ...
      <div className="flex overflow-x-auto mb-8 p-1 -mx-1 gap-2 pb-3 scrollbar-none">
        <button onClick={() => setSelectedTab('overview')} className="px-0">
          Genel Bakış
        </button>
        <button onClick={() => setSelectedTab('participants')} className="px-[23px]">
          Katılımcı Analizi
        </button>
        <button onClick={() => setSelectedTab('timeline')} className="px-[23px]">
          Zaman Çizelgesi
        </button>
        <button onClick={() => setSelectedTab('media')} className="px-[23px]">
          Medya Analizi
        </button>
        <button onClick={() => setSelectedTab('conversation')} className="px-[23px]">
          Konuşma Analizi
        </button>
        <button onClick={() => setSelectedTab('sentiment')} className="px-[23px]">
          Duygu & Manipülasyon
        </button>
        <button onClick={() => setSelectedTab('relationship')} className="px-[23px]">
          İlişki Analizi
        </button>
      </div>
      
      <AnimatePresence mode="wait">
        <motion.div key={selectedTab} initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        duration: 0.3
          ...
          {selectedTab === 'overview' && <div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
                <StatsCard icon={<MessageSquare className="h-5 w-5" />} title="Toplam Mesaj" value={stats.totalMessages} />
                <StatsCard icon={<Type className="h-5 w-5" />} title="Toplam Kelime" value={stats.totalWords} />
                <StatsCard icon={<Smile className="h-5 w-5" />} title="Toplam Emoji" value={stats.totalEmojis} />
                <StatsCard icon={<Calendar className="h-5 w-5" />} title="Sohbet Süresi" value={stats.duration} suffix=" gün" />
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-card rounded-2xl p-6 shadow-soft">
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={messageCountData} cx="50%" cy="50%" innerRadius={isMobile ? 40 : 60} outerRadius={isMobile ? 80 : 100} paddingAngle={2} dataKey="value" animationBegin={0} animationDuration={1000} animationEasing="ease-out" startAngle={90} endAngle={chartProgress * 360 + 90}>
                          {messageCountData.map((entry, index) => <Cell key={`cell-${index}`} fill={participantColors[entry.name]} />)}
                        </Pie>
                        <Tooltip formatter={(value: any) => [`${value} mesaj`, '']} labelFormatter={(label: any) => `${label}`} />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                  <div className="flex flex-wrap gap-3 justify-center mt-2">
                    {Object.entries(participantColors).map(([name, color]) => <div key={name} className="flex items-center gap-2 text-sm">
                ...
                <div className="bg-card rounded-2xl p-6 shadow-soft">
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={wordCountData} layout="vertical" margin={{
                    top: 5,
                    right: 30,
                    left: 50,
                    bottom: 5
                        ...
                        <Bar dataKey="value" animationBegin={0} animationDuration={1500} animationEasing="ease-out">
                          {wordCountData.map((entry, index) => <Cell key={`cell-${index}`} fill={participantColors[entry.name]} />)}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
              ...
              <div className="bg-card rounded-2xl p-6 shadow-soft mb-8">
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={emojiCountData} margin={{
                  top: 5,
                  right: 30,
                  left: 30,
                  bottom: 5
                      ...
                      <Bar dataKey="value" animationBegin={0} animationDuration={1500} animationEasing="ease-out">
                        {emojiCountData.map((entry, index) => <Cell key={`cell-${index}`} fill={participantColors[entry.name]} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
          ...
          {selectedTab === 'participants' && <div>
              <div className="mb-6 flex flex-wrap gap-2">
                {Object.keys(stats.participantStats).map(participant => <button key={participant} onClick={() => setSelectedParticipant(participant)} className={`px-4 py-2 rounded-full btn-transition ${selectedParticipant === participant ? 'bg-primary text-primary-foreground shadow-soft' : 'bg-secondary hover:bg-secondary/80'}`} style={selectedParticipant === participant ? {} : {
              ...
              {selectedParticipant && <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <StatsCard icon={<MessageSquare className="h-5 w-5" />} title="Mesaj Sayısı" value={stats.participantStats[selectedParticipant].messageCount} />
                    <StatsCard icon={<Type className="h-5 w-5" />} title="Kelime Sayısı" value={stats.participantStats[selectedParticipant].wordCount} />
                    <StatsCard icon={<Smile className="h-5 w-5" />} title="Emoji Sayısı" value={stats.participantStats[selectedParticipant].emojiCount} />
                    <StatsCard icon={<Activity className="h-5 w-5" />} title="Ort. Mesaj Uzunluğu" value={Math.round(stats.participantStats[selectedParticipant].averageMessageLength)} suffix=" karakter" />
                  </div>
                  ...
                  <div className="bg-card rounded-2xl p-6 shadow-soft">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {stats.participantStats[selectedParticipant].topEmojis.length > 0 ? stats.participantStats[selectedParticipant].topEmojis.map((emoji, index) => <div key={index} className="bg-secondary/50 rounded-xl p-4 flex flex-col items-center">
                            <div className="text-4xl mb-2">{emoji.emoji}</div>
                            <div className="text-sm text-muted-foreground">{emoji.count} kez</div>
                          </div>) : <div className="col-span-full text-center text-muted-foreground py-4">
                  ...
                  <div className="bg-card rounded-2xl p-6 shadow-soft">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {getParticipantMediaData(selectedParticipant).map((item, index) => <div key={index} className="bg-secondary/50 rounded-xl p-4 text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-secondary/50 rounded-xl p-4">
                        ...
                        <div className="text-2xl font-medium">
                          {stats.participantStats[selectedParticipant].responseTime.average !== null ? `${Math.round(stats.participantStats[selectedParticipant].responseTime.average)} dk` : 'Veri yok'}
                        </div>
                      </div>
                      ...
                      <div className="bg-secondary/50 rounded-xl p-4">
                        ...
                        <div className="text-2xl font-medium">
                          {stats.participantStats[selectedParticipant].responseTime.fastest !== null ? `${Math.round(stats.participantStats[selectedParticipant].responseTime.fastest)} dk` : 'Veri yok'}
                        </div>
                      </div>
                      ...
                      <div className="bg-secondary/50 rounded-xl p-4">
                        ...
                        <div className="text-2xl font-medium">
                          {stats.participantStats[selectedParticipant].responseTime.slowest !== null ? `${Math.round(stats.participantStats[selectedParticipant].responseTime.slowest)} dk` : 'Veri yok'}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {selectedTab === 'timeline' && <div className="space-y-6">
              <div className="bg-card rounded-2xl p-6 shadow-soft">
                ...
                <div className="h-64">
                  ...
                  bottom: 5
                    ...
                    fontSize: 12
                      ...
                      <Line type="monotone" dataKey="count" stroke="#0088FE" strokeWidth={2} dot={{
                    r: 3
                  }} activeDot={{
                    r: 5
                ...
                <div className="h-64">
                  ...
                  bottom: 5
                    ...
                    fontSize: 12
                      ...
                      <YAxis />
                      <Tooltip formatter={(value: any) => [`${value} mesaj`, '']} />
                      <Bar dataKey="count" fill="#00C49F" animationBegin={0} animationDuration={1500} animationEasing="ease-out" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          {selectedTab === 'media' && <div className="space-y-6">
              <div className="bg-card rounded-2xl p-6 shadow-soft">
                <h3 className="text-lg font-medium mb-4">Medya Genel Bakış</h3>
                
                <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-6">
                  <MediaStatsCard title="Fotoğraflar" icon={<Image className="h-5 w-5" />} value={stats.mediaStats.images} color="#4CAF50" />
                  <MediaStatsCard title="Videolar" icon={<Video className="h-5 w-5" />} value={stats.mediaStats.videos} color="#FFC107" />
                  <MediaStatsCard title="Belgeler" icon={<FileText className="h-5 w-5" />} value={stats.mediaStats.documents} color="#2196F3" />
                  <MediaStatsCard title="Linkler" icon={<Link className="h-5 w-5" />} value={stats.mediaStats.links} color="#9C27B0" />
                </div>
                
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                  <MediaStatsCard title="Çıkartmalar" icon={<StickerIcon className="h-5 w-5" />} value={stats.mediaStats.stickers} color="#FF5722" />
                  <MediaStatsCard title="GIFler" icon={<Film className="h-5 w-5" />} value={stats.mediaStats.gifs} color="#795548" />
                  <MediaStatsCard title="Sesler" icon={<Mic className="h-5 w-5" />} value={stats.mediaStats.audio} color="#607D8B" />
                </div>
              </div>
              ...
              <div className="bg-card rounded-2xl p-6 shadow-soft">
                ...
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={mediaData} margin={{
                  top: 20,
                  right: 30,
                  left: 20,
                  bottom: 5
                      ...
                      <Bar dataKey="value" animationBegin={0} animationDuration={1500} animationEasing="ease-out">
                        {mediaData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div className="bg-card rounded-2xl p-6 shadow-soft">
                ...
                <div className="grid gap-6">
                  {Object.entries(stats.participantStats).map(([name, participantStats]) => <div key={name} className="border border-border/30 rounded-xl p-4">
                      <div className="flex items-center mb-4">
                        <div className="w-4 h-4 rounded-full mr-2" style={{
                    backgroundColor: participantColors[name]
                  }}></div>
          ...
          {selectedTab === 'conversation' && <WordAnalysisSection mostFrequentWords={stats.wordAnalysis.mostFrequentWords} mostFrequentWordsByParticipant={stats.wordAnalysis.mostFrequentWordsByParticipant} participantColors={participantColors} mostInitiator={stats.conversationAnalysis.mostInitiator} mostReplier={stats.conversationAnalysis.mostReplier} fastestResponder={stats.conversationAnalysis.fastestResponder} mostDisagreements={stats.conversationAnalysis.mostDisagreements} mostAgreements={stats.conversationAnalysis.mostAgreements} participantStats={stats.participantStats} />}
          
          {selectedTab === 'sentiment' && <SentimentAnalysisSection sentiment={{
          ...
          totalManipulativeMessages: Object.values(stats.participantStats).reduce((total, participant) => total + participant.manipulation.messageCount, 0),
        ...
        }} participantStats={stats.participantStats} participantColors={participantColors} />}
          
          {selectedTab === 'relationship' && <RelationshipAnalysisSection loveExpressions={stats.loveExpressions} apologies={stats.apologies} participantStats={stats.participantStats} participantColors={participantColors} />}
        </motion.div>
      </AnimatePresence>
    </motion.div>;
};
